...
- `@napi-rs/keyring` 通过 N-API 连接 macOS Keychain / Windows Credential Manager；读取必须使用能
  保留存储错误的 `getSecret` API，不能用会把访问拒绝压成空值的旧 password API。现有写入仍保持
  password 形态，以兼容已经保存的 keychain item。Desktop 与独立 CLI 必须在各自 Node/Electron
  运行时加载同一目标平台制品，并用隔离测试 account 验证往返后删除。macOS 的两个入口是不同应用
  身份，安装态还必须覆盖首次授权与权限拒绝，不能只测“密钥缺失”。
