...
从用户价值看，Linnya 要让用户与 Agent 围绕真实工作对象持续完成工作，把材料、上下文、过程和成果积累为可以继续使用的资产。文档数据库是支撑这种工作的产品模型；价值需要由具体任务的完成质量、使用成本和后续复用来检验。
...
具体工作由用户的目标和处境定义，领域插件提供相应的专业能力与真实环境。研究、写作、计算和演示等工作可以独立成立，也可以组合服务一次决策；各自的评价方法由相应领域负责，不要求所有文档都进入同一套决策流程。

分析、比较、判断与执行可以按用户授权逐步自动化。产品的长期价值不以某项能力永远只能由人完成为前提；应保障用户了解系统采用的目标、关键约束和授权范围，并能修改目标、纠正依据、停止后续执行或撤回授权。停止后续执行不等于已经发生的外部行动可以撤销，也不要求每个内部步骤都由用户确认。

用于研究和决策的能力，应将事实与推测、用户目标与价值取舍、建议与执行结果区分开，使新信息和实际反馈能够用于修订认识。其价值要与用户原有方式比较，同时考虑成果质量、时间、注意力和运行成本；一次结果、报告长度或表达说服力不能独立证明系统有效。

基础设施是否开放、产品如何提供、材料是否共享以及成果是否发表，是分别决定的问题；开放代码不构成公开用户工作内容的授权。

### 0.4 最小能力与扩展准则

从一项有边界、能实际完成和检查结果的工作开始验证产品。优先用已有文档、工具和插件完成任务；新增结构化对象、界面或运行机制，应能说明现有方式在哪个真实环节失效，以及增加的维护成本换来了什么改善。

长期通用性来自清楚的领域边界与稳定合同。首版可以只支持有限对象、材料和操作；只有当新场景证明共同需求时，才扩展通用能力。具体场景的首发范围和验证计划不属于本文的永久产品定义。
...
| **接入层** | 插件 contribution：文档类型、VFS 投影、CLI、Skill、富文档交互界面 | **按真实工作需要扩展** |
...
### 4.5 领域插件：专业能力、真实环境与组合

**领域插件真正值得提供的，不应只有页面，而是能帮助系统完成特定工作的专业能力和真实环境。不同的原子化插件可以互相组合，共同解决真实而复杂的工作问题。**

专业能力包括领域知识、业务规则、可执行操作和结果检验方法。插件根据领域需要，通过 §4.1 中彼此独立的贡献方式，让人和 Agent 能够理解工作对象、采取行动并检查结果。页面是其中一种交互入口，插件的产品价值应以它能帮助完成什么工作来衡量。

真实工作环境是这些能力所作用的实际对象、数据、状态与上下文，以及可检查的反馈和可继续使用的成果。例如，表格计算作用于实际表格数据，演示文稿编排作用于实际幻灯片，操作结果能够被保存、查看和后续修改。每个插件按职责提供所需环境，无须各自包办所有环节或拥有独立运行时。

原子化以**可独立理解和使用的业务能力片段**为边界，保留清楚的对象归属、操作语义与生命周期。插件的大小取决于业务内聚性，不能机械地按页面、按钮或技术组件拆分。

组合意味着一个能力的对象、上下文和成果可以经明确的公开合同，被其他能力继续使用；Agent 或应用层工作流据此组织多插件协作。组合必须尊重各领域的业务规则与权限，通过已有 VFS、CLI 等公开入口或明确的跨领域合同完成交接，宿主仍不复制插件语义。跨插件预览或界面嵌入可以按体验需要设计，但不是能力组合的前提。

这是插件体系的长期产品原则；具体可组合的能力及其限制，以各插件和宿主当前的公开合同为准。
...
13. **领域插件提供可组合的专业能力与真实工作环境** —— 插件以可独立使用的业务能力为边界，通过公开合同协作，使工作对象、上下文和成果能够继续参与复杂工作；具体含义见 §4.5。

---

## 7. 文档边界

本文确定通用产品模型与领域边界；具体使用场景的需求强度、最小范围和实际收益仍需任务验证。以下问题属于工程差距与归属分析，不替代这些产品验证，也不改变本文的数据模型：
