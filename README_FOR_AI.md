...
> 本文件是给 AI 与新贡献者的公共工程入口。它只描述公开 Core、开放插件和稳定合同；实现事实以 owner 代码、schema 与相邻 README 为准。

## 1. 产品与架构

Linnya 是基于 Electron 的桌面 Agent 工作台。Workspace、对话式 Agent、富文档编辑、知识库、模型管理和文档插件通过明确的 domain 与 Host 合同协作。
...
| Slides | `packages/plugins/slides/README.md` |
| Conversation CLI | `apps/linnya-cli/README.md` |
| 构建、测试与原生依赖 | `BUILD_AND_TEST_GUIDE.md` |
...
提交前运行目标 owner 的最小真实 gate，并确认 `git diff --check`。完整入口见 [`BUILD_AND_TEST_GUIDE.md`](BUILD_AND_TEST_GUIDE.md)。

## 7. 事实优先级

发生冲突时，依次核对：可执行 schema/public contract 与 owner 实现、相邻稳定 README、本工程地图。历史 proposal、audit、迁移记录和旧 release note 不能覆盖当前合同。

公共仓不保存内部过程 Proposal、研究日志或发布证据，也不得链接或依赖相邻私有文档仓。稳定结论必须回写对应 owner README 或正式规范；完整边界见 [`docs/documentation-governance.md`](docs/documentation-governance.md)。
