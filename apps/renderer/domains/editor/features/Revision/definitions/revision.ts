...
// ==================== Canonical Pending Session（数据库快照索引） ====================
...
export interface CanonicalPendingSession {
  ...
  pendingId: string
  revision: number
  projection: 'deferred' | 'ready' | 'failed'
  /** 目标块 ID */
  blockId: string
  /** 操作类型（insert / update / delete） */
  operation: PendingOperation
  /** 投影到文档后的 revisionId（格式: `ai-${pendingId}-${revision}`） */
  revisionId: string
  /** 后端 pending 的 createdAt */
  createdAt: number
  /**
   * Diff 统计，由 mark 投影完成后回填。
   * 全局 UI 在任一块缺少统计时暂不展示总增删数（块数量仍计入）。
   */
  diffStats?: {
    insertCount: number
    deleteCount: number
...
/** 这里只保存仍待处理的投影；提交完成后随数据库快照移除，不保留终态。 */
export type RevisionStatus = 'pending'
...
export interface RevisionStore {
  bindCommit: (commit: (decision?: MarkdownRevisionCommit['decision'], draft?: ProseMirrorNode) => Promise<void>) => void
  installPendingSnapshot: (pending: readonly WorkspacePendingRevisionDTO[]) => Promise<void>
  readBaseline: (document?: ProseMirrorNode) => ProseMirrorNode
  // ---- Canonical 事实源 ----
  ...
  acceptAllRevisions: (blockId: string) => Promise<void>
  rejectAllRevisions: (blockId: string) => Promise<void>
  /** 文档级：接受当前文档中所有块的待处理修订 */
  acceptAllRevisionsInDocument: () => Promise<void>
  /** 文档级：拒绝当前文档中所有块的待处理修订 */
  rejectAllRevisionsInDocument: () => Promise<void>
...
/** 一次显示投影前后的不可变节点，负责还原结构变换而不改写数据库事实。 */
export interface RevisionProjectionEntry {
  baseline: ProseMirrorNode
  projected: ProseMirrorNode
}
