...
import { scanBlockForRevisions } from '../functions/revisionMarkScan'
...
import { refreshMarkdownDocumentSession, saveMarkdownDocumentSession } from '../../document-session'
...
async function prepareBackendPending(
  ...
  // 先走正式决策撤回旧提议，避免把表格历史投影或旧身份带入新测试数据。
  await refreshMarkdownDocumentSession(editor, documentId)
  if (clearExisting) {
    const revision = useRevisionStore(editor)
    if (revision.canonicalHasAnyPending.value) await revision.rejectAllRevisionsInDocument()
  ...
  console.log('保存文档到后端...')
  await saveMarkdownDocumentSession(editor, documentId)
 ...
 * 完整流程：
 * 1. 如果块不够 → 创建补齐
 * 2. 文档会话保存 → 确保后端 content_json 有这些块（assertBlockExists 需要）
 * 3. set-pending-revisions-batch → 写入后端 pending_revisions 表
 * 4. read-document → 获取真实 DTO
 * 5. 文档会话刷新 → 安装完整正文与 Pending 快照
 */
async function inject(count = 100): Promise<void> {
  ...
  // 与真实外部写入使用同一刷新链路，不单独改写修订缓存。
  await refreshMarkdownDocumentSession(editor, prepared.documentId)
...
async function seed(count = 10000, options: SeedOptions = {}): Promise<void> {
  ...
  const prepared = await prepareBackendPending(count, options)
  if (prepared) {
    console.log(
...
/**
 * 通过正式拒绝流程清除修订，正文和 Pending 在同一事务中更新。
 */
async function clear(): Promise<void> {
  ...
  const revision = useRevisionStore(editor)
  if (revision.canonicalHasAnyPending.value) await revision.rejectAllRevisionsInDocument()
  console.log('[RevisionTest] 已通过文档会话拒绝全部 Pending')
}
...
export { inject, seed, diagnose, diagnoseWindow, dump, clear }
