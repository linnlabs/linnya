...
- CommonMark 不保留空段落；带批注的空 `baseBlock` 会先序列化为不可见的 `linnya-annotation-anchor:v1 empty-block` HTML comment，再输出与其他块完全相同的 Annotation comment。导入时锚点恢复空 rootBlock，批注仍归属该块。
