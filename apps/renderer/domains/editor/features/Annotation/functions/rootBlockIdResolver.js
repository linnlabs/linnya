 ...
 * 小文档里内容 DOM 常驻时，这类错误偶尔还能靠 DOM 查找“看起来能跑”；
...
export function resolveAnnotationRootBlockId(editor, blockId) {
  ...
  const exactRootBlock = document.querySelector(`.root-block-outer[data-id="${escaped}"]`);
  if (exactRootBlock) return blockId;

  const elementWithId = document.querySelector(`[data-id="${escaped}"]`);
