/**
 * @file apps/renderer/shared/utils/textUnits.ts
 *
 * @description
 * 前端统一的“文本长度单位”统计与截断工具。
 *
 * 中文备注（重要）：
 * - 该文件为前端专用实现，**不要引用**后端/主进程 `src/shared/utils/textUnits.ts`；
 * - 计数口径与后端保持一致：中文按“汉字数”计，英文按“单词数”计；
 * - 该工具用于 UI 的长度展示（例如 SharedMemory 阅读卡片 / 编辑器底部的“字数”）。
 *
 * 计数规则（当前版本）：
 * - 中文（汉字，CJK Unified Ideographs）：每个汉字计 1；
 * - 英文/数字：连续的 [A-Za-z0-9] 视为一个“词”，计 1；
 * - 空白与常见标点：不计数（0）；
 * - 其他字符（例如 emoji、其他语种字母）：计 1（保证稳定，不会出现“无单位但无限长”的文本）。
 */
 ...
 * - 对未覆盖的符号按 other=1 计，保证稳定。
 */
// 模块级常量：避免每次调用 isCommonPunctuation 时重复创建 Set
const COMMON_PUNCT_SET: ReadonlySet<string> = new Set<string>([
  '。', '，', '、', '；', '：', '？', '！', '（', '）', '【', '】', '《', '》', '“', '”', '‘', '’', '—', '…',
  '.', ',', ';', ':', '?', '!', '(', ')', '[', ']', '{', '}', '"', "'", '-', '–', '—',
  '/', '\\', '|', '@', '#', '$', '%', '^', '&', '*', '+', '=', '<', '>', '~', '`',
]);
...
function isCommonPunctuation(ch: string): boolean {
  return COMMON_PUNCT_SET.has(ch);
}
...
export function sliceTextByUnitsZhEn(text: string, maxUnits: number): string {
  ...
  for (let i = 0; i < text.length; ) {
    ...
    // 其他字符：计 1
    if (units + 1 > maxUnits) {
      // 如果刚好处在英文单词内部（理论上不会，因为 ascii 分支已 continue），回退到单词开始前
