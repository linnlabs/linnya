...
describe('runCli', () => {
  it.each(['warning', 'error'] as const)(
    ...
    async severity => {
      ...
      await writeFile(
        argsFile,
        JSON.stringify({ locator: 'workspace:/notes.md', content: '大段源码'.repeat(10000) })
      );
      let statusCalls = 0;
  ...
  it('watch 只在状态变化时写 JSONL，并在 awaiting_user 返回控制权', async () => {
    ...
    await expect(
      runCli(['status', 'conversation-1', '--watch', '--interval', '1', '--timeout', '1000'], {
        connection: connect(createClient(execute)),
        io: output.io,
        now: () => 10,
        sleep,
  ...
  it('watch 等待暂停收口后退出，不发送继续或新消息', async () => {
    let callCount = 0;
    const execute: ConversationControlClient['execute'] = async request => {
      if (request.command !== 'status') throw new Error('watch must only observe');
      callCount += 1;
      return {
        schema_version: 1,
        ok: true,
        ...
        run: {
          ...
          agent_id: 'default',
          status: 'paused',
          pause: { settled: callCount > 1, reason: 'user_pause' },
          started_at: 1,
          updated_at: callCount,
          result_available: false,
        },
      };
    };
    ...
    expect(frames).toMatchObject([
      { snapshot: { status: 'paused', pause: { settled: false } } },
      { snapshot: { status: 'paused', pause: { settled: true } } },
    ]);
  });
