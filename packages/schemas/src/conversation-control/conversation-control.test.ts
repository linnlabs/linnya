...
describe('conversation-control wire contract', () => {
  it('models 只返回可复制的选择身份和安全能力，不接纳内部 route', () => {
    ...
    expect(ConversationControlModelsResponseSchema.safeParse({
      ...response,
      chat: [{
        ...response.chat[0],
  ...
  it('连接描述只允许 loopback 和固定长度 session token', () => {
    ...
    expect(ConversationControlConnectionDescriptorSchema.safeParse({
      ...base,
      host: '0.0.0.0',
    }).success).toBe(false);
  });

  it('status/watch 合同不包含主动 paused 或虚构百分比', () => {
    ...
    expect(ConversationControlProgressFrameSchema.parse({
      schema_version: 1,
      frame: 'status',
      sequence: 0,
      observed_at: 210,
      snapshot,
    }).snapshot?.status).toBe('awaiting_user');
    expect(ConversationControlRunStatusSnapshotSchema.safeParse({
      ...snapshot,
      status: 'paused',
    }).success).toBe(false);
    expect(ConversationControlRunStatusSnapshotSchema.safeParse({
  ...
  it('messages 分页合同保持会话级，不接受会破坏游标语义的 run_id', () => {
    expect(ConversationControlMessagesRequestSchema.safeParse({
      schema_version: 1,
      command: 'messages',
      conversation_id: 'conversation-1',
      limit: 80,
  ...
  it('audit 只接受安全聚合，并明确 telemetry 不是完整事实源', () => {
    ...
    expect(ConversationControlAuditResponseSchema.safeParse({
      ...response,
