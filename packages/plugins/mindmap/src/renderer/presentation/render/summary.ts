...
import { generateUUID, getOffsetLT, setAttributes } from '../../shared/utils'
import { createLabel, editSvgText, svgNS } from '../../shared/utils/svg/index'
import { calculatePrecisePosition } from '../../shared/utils/svg/index'
...
const getDirection = function (mind: MindMapInstance, { parent, start }: Summary) {
  const parentEl = mind.findEle(parent)
  const parentObj = parentEl.nodeObj
  let side: DirectionClass
  if (parentObj.parent) {
    side = parentEl.closest('mm-main')!.className as DirectionClass
  } else {
    side = mind.findEle(parentObj.children![start].id).closest('mm-main')!.className as DirectionClass
  }
  return side
}

const drawSummary = function (mind: MindMapInstance, summary: Summary) {
  const { id, label: summaryText, parent, start, end, style } = summary
  const { nodes, theme, summarySvg } = mind
  const parentEl = mind.findEle(parent)
  const parentObj = parentEl.nodeObj
  const side = getDirection(mind, summary)
  let left = Infinity
  let right = 0
  let startTop = 0
  let endBottom = 0
  for (let i = start; i <= end; i++) {
    ...
    if (!child) {
      ...
      mind.removeSummary(id)
      return null
    }
    const wrapper = getWrapper(mind.findEle(child.id))
  ...
  const renderedLabel = mind.markdown ? mind.markdown(summaryText, summary) : summaryText
  if (side === DirectionClass.LEFT) {
  ...
  mind.labelContainer.appendChild(text)
...
export const createSummary = function (this: MindMapInstance, options: SummaryOptions = {}) {
...
export const createSummaryFrom = function (this: MindMapInstance, summary: Omit<Summary, 'id'>) {
...
export const removeSummary = function (this: MindMapInstance, id: string) {
...
export const selectSummary = function (this: MindMapInstance, el: SummarySvgGroup) {
...
export const unselectSummary = function (this: MindMapInstance) {
  this.currentSummary?.labelEl?.classList.remove('selected')
  this.currentSummary = null
}

export const renderSummary = function (this: MindMapInstance) {
...
export const editSummary = function (this: MindMapInstance, el: SummarySvgGroup) {
  if (!el) return
