import { generateUUID, getArrowPoints, getObjById, getOffsetLT, setAttributes } from '../../shared/utils/index'
import LinkDragMoveHelper from '../../shared/utils/dragDrop/LinkDragMoveHelper'
import { calculatePrecisePosition, createArrowGroup, createLabel, editSvgText, svgNS } from '../../shared/utils/svg/index'
import type { CustomSvg, Topic } from '../../domain/types/dom'
import type { MindMapInstance, Uid } from '../../domain/types/index'
import { MINDMAP_RENDER_COLORS } from '../../domain/constants'

const highlightColor = MINDMAP_RENDER_COLORS.linkControllerStroke
 ...
 */
function calcCtrlP(mind: MindMapInstance, tpc: Topic, delta: { x: number; y: number }) {
  const { offsetLeft: x, offsetTop: y } = getOffsetLT(mind.nodes, tpc)
 ...
 * p4: end point
 */
const drawArrow = function (mind: MindMapInstance, from: Topic, to: Topic, obj: Arrow, isInitPaint?: boolean) {
  if (!from || !to) {
    return // not expand
  }

  const fromData = calcCtrlP(mind, from, obj.delta1)
  const toData = calcCtrlP(mind, to, obj.delta2)
  ...
  const { x: halfx, y: halfy } = calcBezierMidPoint(p1x, p1y, p2x, p2y, p3x, p3y, p4x, p4y)
  const labelColor = obj.style?.labelColor || MINDMAP_RENDER_COLORS.arrowLabel
  const groupId = 'arrow-' + obj.id
  newSvgGroup.id = groupId
  const renderedLabel = mind.markdown ? mind.markdown(obj.label, obj) : obj.label
  ...
  mind.labelContainer.appendChild(label)
  mind.linkSvgGroup.appendChild(newSvgGroup)
  calculatePrecisePosition(label)
  if (!isInitPaint) {
    mind.arrows.push(obj)
    mind.currentArrow = newSvgGroup
    showLinkController(mind, obj, fromData, toData)
  }
}

export const createArrow = function (this: MindMapInstance, from: Topic, to: Topic, options: ArrowOptions = {}) {
  const arrowObj = {
    id: generateUUID(),
    label: 'Custom Link',
...
export const createArrowFrom = function (this: MindMapInstance, arrow: Omit<Arrow, 'id'>) {
...
export const removeArrow = function (this: MindMapInstance, linkSvg?: CustomSvg) {
  let link
  if (linkSvg) {
    link = linkSvg
  } else {
...
export const selectArrow = function (this: MindMapInstance, link: CustomSvg) {
...
export const unselectArrow = function (this: MindMapInstance) {
  hideLinkController(this)
...
const hideLinkController = function (mind: MindMapInstance) {
  mind.helper1?.destroy!()
  mind.helper2?.destroy!()
  mind.linkController.style.display = 'none'
  mind.P2.style.display = 'none'
  mind.P3.style.display = 'none'
  if (mind.currentArrow) {
    removeArrowHighlight(mind.currentArrow)
  }
}

const showLinkController = function (mind: MindMapInstance, linkItem: Arrow, fromData: DivData, toData: DivData) {
  const { linkController, P2, P3, line1, line2, nodes, map, currentArrow, bus } = mind
  ...
  mind.helper1 = LinkDragMoveHelper.create(P2)
  mind.helper2 = LinkDragMoveHelper.create(P3)

  mind.helper1.init(map, (deltaX, deltaY) => {
    // recalc key points
    p2x = p2x + deltaX / mind.scaleVal // scale should keep the latest value
    p2y = p2y + deltaY / mind.scaleVal
  ...
  mind.helper2.init(map, (deltaX, deltaY) => {
    p3x = p3x + deltaX / mind.scaleVal
    p3y = p3y + deltaY / mind.scaleVal
...
export function renderArrow(this: MindMapInstance) {
...
export function editArrowLabel(this: MindMapInstance, el: CustomSvg) {
  hideLinkController(this)
  if (!el) return
...
export function tidyArrow(this: MindMapInstance) {
