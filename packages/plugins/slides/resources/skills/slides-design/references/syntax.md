...
`compose({ manualEdits })` 保存已经提交的前端人工值。当前只允许 Text 的完整纯文本 `content` 与各对象的累计 `translation: { dx, dy }`；单位是 inches。记录是当前值，不是不断追加的操作日志；同一页不能重复写同一个 `editKey`。不要手写 arbitrary property path、JSON Patch 或引擎 option。完整、参与 typecheck 的写法见 [`manual-edits.js`](./examples/manual-edits.js)。
