...
export async function verifyRichTextEditing(window: BrowserWindow): Promise<void> {
  ...
  await assert('s.font === "" && s.hasDelete');
  await click(control('.slides-element-property-toolbar__font input'));
  await evaluate('window.shapeTextEditingSmoke.prepareNumberInput(".slides-element-property-toolbar__font input")');
  await window.webContents.insertText('24');
  await key('Enter');
  await click(control('[data-property="text"]'));
  await click(control('[title="#2563EB"]'));
  await assert('s.font === "24" && s.hasDelete && s.commands.length === 1');
  await click('window.shapeTextEditingSmoke.point("standalone")', true);
  await assert('s.draft.every(run => run.style.fontSize === 24 && run.style.color === "#2563EB")');
  for (let i = 0; i < 4; i++) await key('Left', ['shift']);
  await click(control('.slides-element-property-toolbar__font input'));
  await evaluate('window.shapeTextEditingSmoke.prepareNumberInput(".slides-element-property-toolbar__font input")');
  await window.webContents.insertText('32');
  await key('Enter');
  await click('window.shapeTextEditingSmoke.point("second")');
  await click('window.shapeTextEditingSmoke.point("standalone")');
  await assert('s.font === "" && s.hasDelete && s.commands.length === 1');
  await click('window.shapeTextEditingSmoke.point("standalone")', true);
  await assert('s.draft[0].style.fontSize === 24 && s.draft[1].style.fontSize === 32 && s.draft.every(run => run.style.color === "#2563EB")');
  await key('Escape');
  await evaluate('window.shapeTextEditingSmoke.resolve(0, true)');
  await settle();
  await evaluate('window.shapeTextEditingSmoke.present()');
  await settle();
  await assert('s.commands.length === 2 && s.commands[1].op === "set_text_style" && s.font === ""');
  await evaluate('window.shapeTextEditingSmoke.resolve(1, true)');
  await settle();
  await evaluate('window.shapeTextEditingSmoke.present()');
  await settle();
  await assert('s.commands.length === 3 && s.commands[2].op === "set_text_content" && s.font === ""');
  await evaluate('window.shapeTextEditingSmoke.resolve(2, true)');
  await settle();
  await evaluate('window.shapeTextEditingSmoke.present()');
  await settle();
  await assert('s.font === "" && s.hasDelete');
  await click('window.shapeTextEditingSmoke.point("standalone")', true);
  await assert('s.phase === "editing" && s.draft[0].style.fontSize === 24 && s.draft[1].style.fontSize === 32 && s.draft[1].style.color === "#2563EB"');
  ...
  await assert('s.phase === "editing" && s.commands.length === 3');
  await evaluate('window.shapeTextEditingSmoke.richComposition("compositionend")');
  await settle();
  await key('Enter');
  await key('Enter', ['control']);
  await assert('s.phase === "idle" && s.commands.length === 4');
