...
- text writes replace the complete plain-text author value; text style writes merge font size/color; Frame/Shape fill writes replace the solid background/fill; Shape/Image visual-size writes replace width/height; deleting a Frame replaces its record with the exclusive deletion marker;
