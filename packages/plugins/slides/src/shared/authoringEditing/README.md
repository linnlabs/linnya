...
- object-level plain-text style accepts a finite `fontSizePt` from 1 through 400, including decimals, and canonical `#RRGGBB` color values; omitted style fields retain their previous manual value. `SLIDES_MANUAL_FONT_SIZE_PT` is the shared min/max definition consumed by Renderer inputs/presets, operation validation, IPC admission and manual-edit parsing; consumers must not maintain separate range literals;
