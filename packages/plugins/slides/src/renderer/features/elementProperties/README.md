...
While a command or its target visual frame is pending, controls stop emitting additional operations. Selection remains active on the committed frame. The accepted value is already projected by `manualEditing`: text style, fill, size and Frame deletion appear in the current Canvas frame, then stay until the committed RenderModel revision is installed. A backend failure removes that projection and restores the committed pixels.
