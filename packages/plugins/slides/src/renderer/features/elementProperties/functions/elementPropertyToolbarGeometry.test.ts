...
describe('选区属性工具条的可见边界', () => {
  it('选择优先在上方；顶边选择翻到下方', () => {
    ...
    const plain = resolveElementPropertyToolbarPosition({ selection, viewport }, surface);
    expect(plain).not.toBeNull();
    expect((plain?.top ?? 0) + surface.height).toBeLessThan(selection.top);
    const topSelection = { ...selection, top: 0 };
    expect(resolveElementPropertyToolbarPosition({ selection: topSelection, viewport }, surface)?.top).toBeGreaterThan(topSelection.height);
  });
  ...
  it('超大父框和越界子元素的并集保持工具条可触达；完全离开视口才隐藏', () => {
    for (const selection of [
      ...
      const position = resolveElementPropertyToolbarPosition({ selection, viewport }, surface);
    ...
    expect(resolveElementPropertyToolbarPosition({ selection: { left: 610, top: 100, width: 100, height: 50 }, viewport }, surface)).toBeNull();
  });
