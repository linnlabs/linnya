...
- While a command or its complete visual frame is pending, the committed frame remains selectable, so clicking another object is never discarded. New mutations stay gated until the exact revision can accept them; attempted drags during this interval do not create a preview or command.
...
- `orchestration/useSlideManualEditingInteraction.test.ts`: drag-to-pending promotion, parent-first repeated-click descent, stable Frame drag ownership, selection during a mutation gate, and deferred selection after text presentation.
