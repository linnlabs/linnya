...
- Selecting an object exposes the sibling [`elementProperties`](../elementProperties/README.md) feature only when the compiler projects at least one visible property action: plain-text font size/color, Frame/Shape solid color, Shape/Image visual width/height, or the explicit Frame subtree deletion control. The compact toolbar follows the current selected bounds and opens color/size controls on demand; it hides during dragging, resizing and inline text input without changing the canvas viewport. Keyboard-only deletion does not create a toolbar for an otherwise move-only object, and the toolbar does not repeat an author key as a floating title.
