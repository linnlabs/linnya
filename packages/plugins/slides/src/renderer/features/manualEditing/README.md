...
- Double-clicking a plain-string Text or Shape enters [`editingInteraction`](../editingInteraction/README.md), the sole owner of input, IME and interaction handoff. The sibling [`textEditing`](../textEditing/README.md) provides the native DOM view. Clicking another object ends input and selects it immediately while a passive preview retains the submitted text. Submission and frame settlement never hold or reclaim input focus. Multiline and mixed-font strings remain editable when the compiler declares a plain author string.
...
- The same smoke mounts production `ManualResizeHandles` and `ElementPropertyToolbar`; Electron native pointer input verifies intermediate preview without submission, one submission per release, a consecutive resize before revision installation, Escape rollback, click-without-drag and keyboard focus restoration. It saves theme screenshots under `dist/dev/manual-properties-*.png`. Localization uses fixture fallback text; no user database or backend write is involved.
