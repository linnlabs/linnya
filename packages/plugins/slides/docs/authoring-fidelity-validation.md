...
确认 `vue-konva@3.4.0` 非严格属性缓存导致 纯色 → 渐变 → 相同纯色 时实际 fill 丢失。
数据库背景正确；单页新建 stage 的 raster 无法发现这个问题。修复将主预览全部声明式节点切为严格
属性同步，不改变文稿源码、版本或数据库。

真实 Vue/Electron 回归在修复前第三帧报 fill=undefined，修复后通用 16 帧比较通过。另只读提取本地
16 页文稿的背景 Paint，按正序、逆序、再正序执行，背景和 shape 两条路径的 96 帧均与新建实例像素
一致。这是背景状态转移验收，不代表原始文稿的完整内容、P0 或跨平台验收已完成。
