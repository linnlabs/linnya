...
Editor 在自己的容器上保留 mousedown 选区策略，并使用业务标记识别自己的表面。含原生输入的消费者不继承这一策略。显示条件、选区几何、越界定位和业务命令属于各自 owner；不要把 Editor extension、registry 或 Slides 作者合同送入共享包。Editor 原位颜色面板及修订动作仍留在 Editor。
