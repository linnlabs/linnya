 ...
 * **输入 (Input):** PDF二进制数据
 * **输出 (Output):** 处理结果
 * **副作用 (Side-effects):** 使用pdf-parse库提取文本
 */

import { ParsedBlock } from '../../types';
import { StrategyResult, TextItem } from '../types';
import { extractWithPdfParse, evaluateExtractionQuality } from '../adapters/PdfParseAdapter';
 ...
 */
export async function tryQuickTextExtraction(data: Uint8Array, docId: string): Promise<StrategyResult> {
  try {
    console.log('[TextExtractionStrategy] 开始快速文本提取...');
    
    // 使用pdf-parse进行文本提取
    const pdfData = await extractWithPdfParse(data);
    
    // 评估提取质量
    const qualityResult = evaluateExtractionQuality(pdfData);
    console.log(`[TextExtractionStrategy] pdf-parse结果: ${pdfData.numpages}页, 文本密度: ${qualityResult.textDensity.toFixed(1)} 字符/页`);
    ...
    return {
      success: false,
      error: `pdf-parse失败: ${error instanceof Error ? error.message : String(error)}`
    };
  }
}
...
export function detectTextPatterns(
  ...
  const customMatches = patterns.map(pattern => text.match(pattern)).filter(Boolean) as RegExpMatchArray[];
