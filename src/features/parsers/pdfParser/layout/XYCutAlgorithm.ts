 ...
 */
export async function extractTextWithXYCut(
  page: any, 
  columnCount: number
): Promise<string> {
  ...
  const columns: TextItem[][] = Array(xPeaks.length).fill(null).map(() => []);
  ...
  const sortedColumns = columns.map(column => 
    column.sort((a, b) => b.transform[5] - a.transform[5]) // y坐标降序
  );

  // 3. 按从左到右的顺序连接各栏
  return sortedColumns
    .map(column => column.map(item => item.str).join(' '))
    .join('\n\n');
}
 ...
 * **副作用 (Side-effects):** 无副作用，纯提取函数
 */
export async function extractTextDirect(page: any): Promise<string> {
  const textContent = await page.getTextContent();
  const items = textContent.items as TextItem[];
 ...
 */
export function mergeColumnsToText(
  columns: TextItem[][], 
  separateColumns: boolean = true
): string {
