...
`write_file` / `edit_file` 不声明按对话或 turn 的同参幂等缓存：文件在两次调用之间可以被编辑、接受/拒绝 pending revision 或恢复版本。每次调用都必须基于当前内容执行。写入 A → B → A 应真正恢复 A；编辑 A → B → A → B 的最后一步也必须重新执行，不能复用第一次的成功结果。底层精确替换与文档写入事务仍由原 owner 负责。
