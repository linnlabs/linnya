...
失败信息应说明可确认的原因和下一步动作；参数错误提供能通过同一 owner admission 的最小示例，
不能回显用户提交的长正文。不要为了统一形状把所有异常包装成一个通用错误码，也不要从
自然语言反推 `retryable`。取消/恢复控制异常、文档冲突、外部不可取得和普通程序异常保持各自语义。
工具结果摘要解析失败时只能显示“无法确认”，不能兜底显示“完成”；成功写入与内容已核验是不同事实。
...
- `todo_write` 同样不声明历史同参幂等：它替换当前整表快照，每次新调用从最新成功事实递增版本；A→B→A 是三次写入意图。恢复同一已完成 call 应复用该 call 的执行事实，而不是按参数命中另一条历史调用。
