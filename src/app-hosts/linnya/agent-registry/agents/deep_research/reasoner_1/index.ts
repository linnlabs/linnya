/**
 * @file src/app-hosts/linnya/agent-registry/agents/deep_research/reasoner_1/index.ts
 * @description Deep Research Reasoner_1 AgentDefinition（Phase 2：多角色编排）
 *
 * 中文备注：
 * - Reasoner_1 的职责是“初步推理 + 阶段性收敛”：能在本步收敛的就收敛；收敛不了的用未决点/证据缺口中性表达；
...
import { DEEP_RESEARCH_REASONER_1_PROMPT } from './prompt';
...
function buildSystemPrompt(): string {
  return buildPrompt(DEEP_RESEARCH_REASONER_1_PROMPT, {
...
export const AGENT_DEFINITION: AgentDefinition = {
  ...
  defaultMode: 'agent',
  description: 'Deep Research Reasoner_1 - 初步推理与收敛（输出阶段性结论与未决点）',
  config: {
    contextPolicy: {
      profileId: 'agent',
    ...
    /**
     * ✅ 新流程：Reasoner_1/2 允许有限次“深度搜索/补证据”，并负责更新案情板。
     *
     * 中文备注：
     * - 仍然强调：Reasoner_1 的主体工作是推理与初步收敛，检索只是为补关键证据服务；
     * - 真正的“次数限制”由提示词约束（Phase 2 先不在 runner 强制预算）。
     */
    enableTools: true,
    availableTools: [
     ...
     *
     * 中文备注：
     * - Reasoner_1 在尾段应优先：读板对齐 → 补关键证据（必要时）→ 提交 board_propose → 输出阶段性结论；
     * - 这里用提示阈值提醒模型“剩余步数”，避免临界点还在发散检索。
     *
     * 根因级说明：
     * - Reasoner_1 的 prompt 明确要求通过 write_file 更新 `workspace:/research-board.md`；
