...
describe('Conversation Agent control-plane guard', () => {
  ...
  it.each([
    ...
    ['assertion wrapper', 'const key = (conversation.metadata as { promptKey?: string }).promptKey;'],
    ['write', "conversation.metadata.promptKey = 'slides_agent';"],
  ])('识别 metadata.promptKey %s', (_name, source) => {
    expect(analyzeConversationAgentControlPlane(CONVERSATION_FILE, source))
      .toEqual([expect.objectContaining({ ruleId: 'AGENT-CONTROL-01-metadata-prompt-key' })]);
  });
  ...
  it('识别 metadata 解构与 merge patch 中的 promptKey', () => {
    ...
    expect(analyzeConversationAgentControlPlane(CONVERSATION_FILE, source).map(item => item.line))
      .toEqual([1, 2, 3]);
  });
  ...
  it('识别 conversationAgentChoices 内部 promptKey，不误报 subrun worker', () => {
    ...
    expect(analyzeConversationAgentControlPlane(PLUGIN_FILE, source))
  ...
  it('解析 Vue script 并报告真实行号', () => {
    ...
    expect(analyzeConversationAgentControlPlane(
