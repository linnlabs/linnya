...
import { serializePosixCommandArgv } from '../../../../../src/infra/adapters/command-runtime/macos/functions/serializePosixCommandArgv';
...
export async function runProductionAgentPtyScenario(input: {
  ...
  readonly interactiveCliSourcePath: string;
  ...
  const workDirectory = await lifecycle.workDirectoryAdmission.withAdmission(
    ...
    async (directory) => {
      const cliPath = path.join(directory.absolutePath, 'interactive-cli.cjs');
      await fsp.copyFile(input.interactiveCliSourcePath, cliPath);
  ...
  const command = serializePosixCommandArgv(input.nodeExecutable, [
    './interactive-cli.cjs',
    input.runToken,
    cliIdentityPath,
  ]);
  ...
  try {
    const result = await orchestrator.next({
      ...
      new_events: [{
        type: 'user_input',
        content: '启动交互命令，提交输入，调整尺寸，并发送 EOF。',
        timestamp: Date.now(),
        source: 'user',
        turn_id: TURN_ID,
      }],
    ...
    assert(result.events.some(event => (
      ...
      && event.content === '交互命令已提交输入、调整终端尺寸，并在 EOF 后自然退出。'
    )));
    ...
    assert(
      commandMessages.length >= 8 && commandMessages.length <= 21,
    ...
    assert.equal(cardData['terminal']['outcome'], 'exited');
    assert.equal(cardData['terminal']['exitCode'], 0);
    ...
    assert(
      projectedScreenText.includes('PTY_SCREEN_SAFE_中'),
      `PTY screen lost the safe corpus marker: ${JSON.stringify(projectedScreenText)}`,
    );
    for (const forbidden of [
      ...
      assert(
        !projectedScreenText.includes(forbidden),
        `PTY screen leaked control payload: ${forbidden}`,
      );
    }
    ...
    const rawTerminal = await fsp.readFile(path.join(artifactDirectory, 'terminal.bin'));
    assert(rawTerminal.includes(Buffer.from('PTY_READY:true:true')), 'raw PTY artifact lost the TTY marker');
    assert(rawTerminal.includes(Buffer.from('SUBMIT_OK:accepted-value')), 'raw PTY artifact lost submitted input');
    assert(rawTerminal.includes(Buffer.from('PTY_SIZE:100x31')), 'raw PTY artifact lost resized dimensions');
    assert(rawTerminal.includes(Buffer.from('PTY_EOF_NATURAL_EXIT')), 'raw PTY artifact lost the EOF marker');
    for (const preservedPayload of [
      '\u001b]2;PC53_PTY_TITLE_SECRET\u0007',
      '\u001b]52;c;PC53_PTY_CLIPBOARD_SECRET\u0007',
      '\u001b]8;;https://pc53.invalid/secret\u001b\\',
      '\u001b]9;PC53_PTY_NOTIFICATION_SECRET\u0007',
      '\u001b]1337;File=name=PC53_PTY_IMAGE_SECRET:AAAA\u0007',
      '\u001bPPC53_PTY_DCS_SECRET\u001b\\',
    ]) {
      assert(
        rawTerminal.includes(Buffer.from(preservedPayload)),
        `raw PTY artifact lost control payload: ${JSON.stringify(preservedPayload)}`,
      );
    }
    ...
    return {
      ...
      terminalExitCode: cardData['terminal']['exitCode'],
      ...
      cliParentPid: cliIdentity['parentPid'],
    };
  } finally {
