...
const observationReadyPath = requireAbsoluteEnvironmentPath('LINNYA_AGENT_PTY_OBSERVATION_READY_PATH');
const workDirectoryHandshakePath = requireAbsoluteEnvironmentPath('LINNYA_AGENT_PTY_WORK_DIRECTORY_HANDSHAKE_PATH');
const command = requireEnvironmentValue('LINNYA_AGENT_PTY_COMMAND');
const runToken = requireEnvironmentValue('LINNYA_AGENT_PTY_RUN_TOKEN');
...
async function main(): Promise<void> {
  ...
  if (effectiveUserDataRoot !== path.resolve(userDataRoot)) {
    throw new Error(`Electron userData escaped the isolated run root: ${effectiveUserDataRoot}`);
  }
  ...
  publishResult(await runProductionAgentPtyScenario({
    ...
    runnerRoot: process.env.LINNYA_AGENT_PTY_RUNNER_ROOT ?? path.join(__dirname, 'commands'),
  }));
  app.quit();
}
