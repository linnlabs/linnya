...
const fixtureDirectory = fileURLToPath(
  new URL('./fixtures/windows-production-agent-process/', import.meta.url)
);
const sharedFixtureDirectory = fileURLToPath(
  new URL('./fixtures/production-agent-process/', import.meta.url)
);
const controllerPath = fileURLToPath(
  new URL('./windows-production-agent-process.e2e.ps1', import.meta.url)
);
...
if (process.platform !== 'darwin') {
  throw new Error('Windows production Agent process E2E must be orchestrated from macOS');
}
...
async function createBuildProject(isolatedRoot) {
  ...
  await runChecked('pnpm', [
    'exec',
    'esbuild',
    path.join(sharedFixtureDirectory, 'main.ts'),
    '--bundle',
    '--platform=node',
    '--format=cjs',
    '--target=node20',
  ...
  await copyFile(
    path.join(fixtureDirectory, 'production-agent-process-tree.ps1'),
    path.join(projectDirectory, 'command-fixtures/production-agent-process-tree.ps1')
  );
...
async function inspectPackage(outputDirectory) {
  const executablePath = path.join(
    outputDirectory,
    'win-unpacked/Linnya Windows Production Agent Process Validation.exe'
  );
  ...
  try {
    ...
    assert(
      packagedMain.includes('runProductionAgentProcessScenario'),
      'packaged main.cjs is missing the production Agent scenario'
    );
  } finally {
  ...
  for (const filePath of [
    ...
    path.join(resourcesPath, 'command-fixtures/production-agent-process-tree.ps1'),
  ]) {
    await stat(filePath);
  }
...
function assertSummary(summary) {
  ...
  assert.equal(summary.durableCommandRows, 5);
  ...
  assert.equal(summary.externallyObservedProcesses, 3);
  assert.equal(summary.externallyExitedProcesses, 3);
  assert.equal(summary.allObservedProcessesInJob, true);
  assert.equal(summary.childParentVerified, true);
  assert.equal(summary.grandchildParentVerified, true);
  assert.equal(summary.heartbeatStopped, true);
  assert.equal(summary.promptLikeOutputObserved, true);
  assert.equal(summary.childStdinEofObserved, true);
  assert.equal(summary.silentWaitReturnedRunning, true);
  assert.equal(summary.automaticInputActions, 0);
}
...
async function runWindowsValidation(projectDirectory, buildRoot) {
  ...
  const archivePath = path.join(buildRoot, `windows-production-agent-${token}.tar.gz`);
...
const isolated = await createIsolatedRunRoot('windows-production-agent-process');
let success = false;
try {
  ...
  if (!success) {
    process.stderr.write('Windows production Agent process E2E failed after isolated cleanup\n');
  }
}
